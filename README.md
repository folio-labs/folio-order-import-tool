...
| **Server**                ||||
| uploadFilePath            | file system path. The leading sections of the path must exist on the server, the last section (ie `order-import`) will be created if allowed. ² | /tmp/jetty/order-import | Defines where uploaded MARC files are stored on the server.                                                                                                                                                                                                                                                           |
| **Processing**            ||||
