...
public class TestImportOrderFirst {
	
	private static final Logger logger = Logger.getLogger(TestImportOrderFirst.class);
	...
	public  JSONObject  upload(String fileName) throws IOException, InterruptedException, Exception {
		...
		if (nineEighty == null) {
			responseMessage.put("error", "File is missing the 980 field");
			responseMessage.put("PONumber", "~error~");
			return responseMessage;
		}
	    ...
	    if (objectCode == null) {
		    responseMessage.put("error", "Object Code Missing");
			responseMessage.put("PONumber", "~error~");
			return responseMessage;
		}
	    
	    if (fundCode == null) {
		    responseMessage.put("error", "Fund Code Missing");
			responseMessage.put("PONumber", "~error~");
			return responseMessage;
		}
	    
	    if (vendorCode == null) {
		    responseMessage.put("error", "Organization Code Missing");
			responseMessage.put("PONumber", "~error~");
			return responseMessage;
		}
	    
	    if (price == null) {
		    responseMessage.put("error", "Price is Missing");
			responseMessage.put("PONumber", "~error~");
			return responseMessage;
		}
	    
	    //VALIDATE INFO FROM THE 980
	    //1 MAKE SURE IT IS A VALID FUND CODE
	    //2 MAKE SURE THE FUND CODE HAS ENOUGH MONEY
	    //3 VALIDATE THE OBJECT CODE
	    //4 VALIDATE THE VENDOR
		...
		if (orgObject.getJSONArray("organizations").length() < 1) {
			responseMessage.put("error", "Organization code in file (" + vendorCode + ") does not exist in FOLIO");
			responseMessage.put("PONumber", "~error~");
			return responseMessage;
		}
		...
		//----------->VALIDATION #2
		String fundBalanceQuery = baseOkapEndpoint + "finance/budgets?query=(fundId=="  + fundId + ")";
		...
		//MAKE SURE THE OBJECT CODE EXISTS IN FOLIO
		//https://folio-test.lib.lehigh.edu/okapi/tags?query=(label==OBJ-CD-EBFR)
		String tagEndpoint = baseOkapEndpoint + "tags?query=(label==" + objectCode + ")";
		...
		if (tagObject.getJSONArray("tags").length() < 1) {
			responseMessage.put("error", "Object code in file (" + objectCode + ") does not exist in FOLIO");
			responseMessage.put("PONumber", "~error~");
			return responseMessage;
		}
		...
		((ControlField) record.getVariableField("001")).setData(hrid);
		
		try {
			((ControlField)record.getVariableField("005")).setData(timeStamp + ".2");
		}
		catch(Exception e) {
			ControlField cf = MarcFactory.newInstance().newControlField();
			cf.setTag("005");
			cf.setData(timeStamp + ".2");
